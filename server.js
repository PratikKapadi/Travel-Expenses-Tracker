...
app.use(
  session({
    secret:
      "3c51b23811909e39a96f06d9f5d1cfb9e95ad9a04b63a6d83f72612eb001c81a7cca39e1a1068d6b189ce8e7ae6fcbdc778e2153eff1890bbfb01be6164619d2", // Use a secure random string here
    resave: false,
...
app.post("/login", async (req, res) => {
  ...
  if (results.length > 0) {
    ...
    res.send(
      `<script>alert('${errorMessage}'); window.location='/login';</script>`
    );
  }
});
...
app.post("/forgotpassword", async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      await connection2.query("UPDATE users SET password = ? WHERE email = ?", [
        password,
        email,
      ]);
      connection2.release();
...
app.post("/addtrip", async (req, res) => {
  ...
  try {
    ...
    const query =
      "INSERT INTO trips (username, start_date, end_date, place, country, trip_type) VALUES (?, ?, ?, ?, ?, ?)";
...
app.post("/addexpenses", async (req, res) => {
  ...
  const query =
    "INSERT INTO expenses (username, date, category, amount, description, tripname) VALUES (?, ?, ?, ?, ?, ?)";
  await pool.query(query, [
    username,
    date,
    category,
...
app.post("/admin_login", async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 1) {
      ...
      res.send(
        '<script>alert("Invalid username or password"); window.location="/index_admin";</script>'
      );
    }
  } catch (error) {
    console.error(error);
...
app.get("/admin", (req, res) => {
  if (req.session.email) {
    res.send(
      `<h1>Welcome, ${req.session.email}!</h1><a href="/admin_logout">Logout</a>`
    );
  } else {
    res.redirect("/admin");
  }
...
app.get("/generate-report", async (req, res) => {
  try {
    ...
    const [trips] = await pool.query(
      "SELECT * FROM trips WHERE MONTH(start_date) = ? AND YEAR(start_date) = ?",
      [month, year]
    );
